              ...
              itemBuilder: (_, i) => GestureDetector(
                  ...
                  child: AnimatedContainer(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(
                            color:
                                _tab == i ? AppTheme.white : AppTheme.border),
                        borderRadius: BorderRadius.circular(3),
                      ),
...
class _PhotosTabState extends State<_PhotosTab> {
  ...
  Future<void> _pick() async {
    ...
    if (r == null) return;
    final valid =
        r.files.take(10).where((f) => f.size <= 10 * 1024 * 1024).toList();
    if (valid.isEmpty) {
      _snack('All files exceed 10MB.', isError: true);
      return;
    }
    setState(() {
      _tasks =
          valid.map((f) => _UploadTask(name: f.name, bytes: f.bytes!)).toList();
      _uploading = true;
    });
    for (int i = 0; i < _tasks.length; i++) {
      setState(
          () => _tasks[i] = _tasks[i].copyWith(status: _UStatus.uploading));
      ...
      setState(() => _tasks[i] = _tasks[i].copyWith(
          status: res.success ? _UStatus.done : _UStatus.error,
          error: res.error));
    }
  ...
  void _snack(String msg, {bool isError = false}) =>
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          ...
          shape: RoundedRectangleBorder(
              ...
              side: BorderSide(
                  color:
                      isError ? const Color(0xFFFF5555) : AppTheme.border))));
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (_tasks.isNotEmpty) ...[
            ...
            ..._tasks
                .asMap()
                .entries
                .map((e) => _UploadRow(task: e.value, index: e.key)),
            if (!_uploading)
              TextButton(
          ...
          const _SectionTitle(
              title: 'Manage Photos', sub: 'Hover a photo and click Remove'),
          const SizedBox(height: 20),
          StreamBuilder<List<PhotoEntry>>(
              ...
              builder: (_, snap) {
                ...
                if (photos.isEmpty)
                  return const _EmptyBox(label: 'No photos yet');
                    ...
                    itemBuilder: (_, i) => _ManagePhotoCard(
                        ...
                        onDelete: () async {
                          final ok = await showDialog<bool>(
                              ...
                              false;
                          if (ok) {
                            await FirebaseService.removePhoto(photos[i].docId!);
                            _snack('Removed.');
                          }
                        }));
              }),
...
class _ProfileTabState extends State<_ProfileTab> {
  ...
  Future<void> _pickAndUpload() async {
    ...
    if (file.size > 10 * 1024 * 1024) {
      setState(() => _status = '✗ File exceeds 10MB.');
      return;
    }
    setState(() {
      _uploading = true;
    ...
    try {
      final res = await CloudinaryService.uploadProfilePhoto(
          ...
          .timeout(const Duration(seconds: 30),
              onTimeout: () =>
                  CloudinaryUploadResult(success: false, error: 'Timed out.'));
      if (!mounted) return;
      if (res.success && res.url != null) {
        ...
        setState(() {
          _uploading = false;
          _status = '✗ ${res.error ?? 'Upload failed'}';
        });
      }
    } catch (e) {
      if (mounted)
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const _SectionTitle(
              title: 'Profile Photo',
              sub: 'Upload a new photo and adjust the crop'),
          ...
          Center(
            ...
            const Text(
                ...
                style: TextStyle(
                    fontSize: 11, color: AppTheme.greyDark, height: 1.6)),
          ])),
          const SizedBox(height: 60),
        ]));
  }
...
class _SkillsTabState extends State<_SkillsTab> {
  ...
  Future<void> _save() async {
    ...
    if (name.isEmpty) {
      _snack('Enter a skill name.', isError: true);
      return;
    }
    if (_editing != null) {
      final err = await FirebaseService.updateSkill(
          _editing!.docId!, SkillEntry(name: name, category: _category));
      if (err != null) {
      ...
      final err = await FirebaseService.addSkill(
          SkillEntry(name: name, category: _category));
      if (err != null) {
        _snack('Error: $err', isError: true);
        return;
      }
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          _SectionTitle(
              title: _editing != null ? 'Edit Skill' : 'Add Skill',
              sub: 'Changes appear live on the Skills page'),
          if (_editing != null)
            _EditingBanner(
                label: 'Editing: ${_editing!.name}',
                onCancel: () {
                  setState(() => _editing = null);
                  _nameCtrl.clear();
                }),
          ...
          const SizedBox(height: 20),
          _SaveButton(
              label: _editing != null ? 'Update Skill' : 'Add Skill',
              onTap: _save),
          const SizedBox(height: 40),
          const _SectionTitle(
              title: 'All Skills',
              sub: 'Tap the edit icon to modify any skill'),
          const SizedBox(height: 16),
          StreamBuilder<List<SkillEntry>>(
              ...
              builder: (_, snap) {
                ...
                if (items.isEmpty)
                  return const _EmptyBox(label: 'No skills yet');
                return Column(
                    children: items
                        .map((s) => _EditableRow(
                              label: s.name,
...
class _CertsTabState extends State<_CertsTab> {
  ...
  Future<void> _pickImage() async {
    ...
    if (f.size > 10 * 1024 * 1024) {
      _snack('File exceeds 10MB.', isError: true);
      return;
    }
    setState(() {
      _imgBytes = f.bytes;
  ...
  Future<void> _save() async {
    final name = _nameCtrl.text.trim();
    if (name.isEmpty) {
      _snack('Enter a certificate name.', isError: true);
      return;
    }
    ...
    if (_imgBytes != null && _imgFileName != null) {
      ...
      final res = await CloudinaryService.uploadCertificateImage(
          bytes: _imgBytes!, fileName: _imgFileName!);
      if (!res.success) {
        setState(() => _saving = false);
        _snack('Image upload failed: ${res.error}', isError: true);
        return;
      }
      imageUrl = res.url;
    }
    final issuer = _issuer == 'New'
        ? (_customCtrl.text.trim().isEmpty ? 'Other' : _customCtrl.text.trim())
        : _issuer;
    String? err;
    if (_editing != null) {
      err = await FirebaseService.updateCert(_editing!.docId!,
          CertEntry(name: name, issuer: issuer, imageUrl: imageUrl));
      if (err == null) {
        setState(() => _editing = null);
        _snack('Certificate updated!');
      }
    } else {
      err = await FirebaseService.addCert(
          CertEntry(name: name, issuer: issuer, imageUrl: imageUrl));
      if (err == null)
        _snack('Certificate added!${imageUrl != null ? ' (with image)' : ''}');
    }
  ...
  void _startEdit(CertEntry c) {
    setState(() {
      _editing = c;
      _issuer = ['MMCL', 'Coursera'].contains(c.issuer) ? c.issuer : 'New';
    });
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          _SectionTitle(
              title: _editing != null ? 'Edit Certificate' : 'Add Certificate',
              sub:
                  'Upload a JPG/PNG image of your certificate to make it viewable'),
          if (_editing != null)
            _EditingBanner(
                label: 'Editing: ${_editing!.name}',
                onCancel: () {
          ...
          _StyledTextField(
              controller: _nameCtrl, hint: 'e.g. Introduction to AI'),
          const SizedBox(height: 12),
          const _FieldLabel('Issuer / Section'),
          Wrap(
              spacing: 8,
              runSpacing: 8,
              children: ['MMCL', 'Coursera', 'New']
                  .map((s) => _ChoiceChip(
                      label: s == 'New' ? '+ New Section' : s,
                      selected: _issuer == s,
                      onTap: () => setState(() => _issuer = s)))
                  .toList()),
          if (_issuer == 'New') ...[
            const SizedBox(height: 10),
            _StyledTextField(
                controller: _customCtrl, hint: 'Section name (e.g. Google)')
          ],
          const SizedBox(height: 14),
          ...
          Row(children: [
            ...
            if (_editing?.hasImage == true && _imgBytes == null) ...[
              const SizedBox(width: 8),
              const Text('Current image kept',
          ...
          _SaveButton(
              label:
                  _editing != null ? 'Update Certificate' : 'Add Certificate',
              loading: _saving,
              onTap: _saving ? null : _save),
          const SizedBox(height: 40),
          const _SectionTitle(
              title: 'All Certificates', sub: 'Tap the edit icon to modify'),
          const SizedBox(height: 16),
          StreamBuilder<List<CertEntry>>(
              ...
              builder: (_, snap) {
                final items = snap.data ?? [];
                if (items.isEmpty)
                  return const _EmptyBox(label: 'No certificates yet');
                return Column(
                    children: items
                        .map((c) => _EditableRow(
                              label: c.name,
                              sublabel:
                                  '${c.issuer}${c.hasImage ? '  ·  Has PDF' : ''}',
                              onEdit: () => _startEdit(c),
                              onDelete: () async {
                                final ok = await showDialog<bool>(
                                        context: context,
...
class _ProjectsTabState extends State<_ProjectsTab> {
  ...
  Future<void> _save() async {
    ...
    if (name.isEmpty) {
      _snack('Enter a project name.', isError: true);
      return;
    }
    if (url.isEmpty) {
      _snack('Enter a project URL.', isError: true);
      return;
    }
    ...
    String? err;
    if (_editing != null) {
      err = await FirebaseService.updateProject(_editing!.docId!, p);
      if (err == null) {
        setState(() => _editing = null);
        _snack('Project updated!');
      }
    } else {
      err = await FirebaseService.addProject(p);
      if (err == null) _snack('Project added!');
    }
    if (err != null) {
      _snack('Error: $err', isError: true);
      return;
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          _SectionTitle(
              title: _editing != null ? 'Edit Project' : 'Add Project',
              sub: 'Changes appear live on the Projects page'),
          if (_editing != null)
            _EditingBanner(
                label: 'Editing: ${_editing!.name}',
                onCancel: () {
          ...
          _StyledTextField(
              controller: _descCtrl, hint: 'Short description', maxLines: 3),
          const SizedBox(height: 10),
          const _FieldLabel('Tags (comma-separated)'),
          _StyledTextField(
              controller: _tagsCtrl, hint: 'React, Vercel, Web App'),
          const SizedBox(height: 12),
          const _FieldLabel('Status'),
          Wrap(
              spacing: 8,
              runSpacing: 8,
              children: ['Live', 'In Progress', 'Archived']
                  .map((s) => _ChoiceChip(
                      label: s,
                      selected: _status == s,
          ...
          const SizedBox(height: 20),
          _SaveButton(
              label: _editing != null ? 'Update Project' : 'Add Project',
              onTap: _save),
          const SizedBox(height: 40),
          const _SectionTitle(
              title: 'All Projects', sub: 'Tap the edit icon to modify'),
          const SizedBox(height: 16),
          StreamBuilder<List<ProjectEntry>>(
              ...
              builder: (_, snap) {
                final items = snap.data ?? [];
                if (items.isEmpty)
                  return const _EmptyBox(label: 'No projects yet');
                return Column(
                    children: items
                        .map((p) => _EditableRow(
                              label: p.name,
...
class _HobbiesTab extends StatefulWidget {
  const _HobbiesTab();
  @override
  State<_HobbiesTab> createState() => _HobbiesTabState();
}
...
class _HobbiesTabState extends State<_HobbiesTab> {
  ...
  Future<void> _save() async {
    final name = _nameCtrl.text.trim();
    if (name.isEmpty) {
      _snack('Enter a hobby name.', isError: true);
      return;
    }
    String? err;
    if (_editing != null) {
      err = await FirebaseService.updateHobby(
          _editing!.docId!,
          HobbyEntry(
              name: name, desc: _descCtrl.text.trim(), order: _editing!.order));
      if (err == null) {
        setState(() => _editing = null);
        _snack('Hobby updated!');
      }
    } else {
      err = await FirebaseService.addHobby(
          HobbyEntry(name: name, desc: _descCtrl.text.trim()));
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          _SectionTitle(
              title: _editing != null ? 'Edit Hobby' : 'Add Hobby',
              sub: 'Changes appear live on the Hobbies page'),
          if (_editing != null)
            _EditingBanner(
                label: 'Editing: ${_editing!.name}',
                onCancel: () {
          ...
          const SizedBox(height: 20),
          _SaveButton(
              label: _editing != null ? 'Update Hobby' : 'Add Hobby',
              onTap: _save),
          const SizedBox(height: 40),
          const _SectionTitle(
              title: 'All Hobbies', sub: 'Tap the edit icon to modify'),
          const SizedBox(height: 16),
          StreamBuilder<List<HobbyEntry>>(
              ...
              builder: (_, snap) {
                final items = snap.data ?? [];
                if (items.isEmpty)
                  return const _EmptyBox(label: 'No hobbies yet');
                return Column(
                    children: items
                        .map((h) => _EditableRow(
                              label: h.name,
                              ...
                              onDelete: () async {
                                final ok = await showDialog<bool>(
                                    ...
                                    false;
                                if (ok) {
                                  await FirebaseService.removeHobby(h.docId!);
                                  _snack('Removed.');
                                }
                              },
                            ))
...
class _ReviewsTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final isMobile = MediaQuery.of(context).size.width < 768;
    return StreamBuilder<List<ReviewEntry>>(
        ...
        return SingleChildScrollView(
          ...
          child:
            ...
            const _SectionTitle(
                ...
                sub: 'Remove any review you do not want shown publicly'),
            const SizedBox(height: 16),
            if (items.isEmpty)
              const _EmptyBox(label: 'No reviews yet')
            else
              ...items.map((r) => _EditableRow(
                    label: r.name,
                    sublabel: '${r.comment}  ·  ${r.rating}★',
                    onEdit: () {},
                    onDelete: () async {
                      final ok = await showDialog<bool>(
                              context: context,
                          ...
                          false;
                      if (ok) await FirebaseService.removeReview(r.docId!);
                    },
                  )),
            const SizedBox(height: 60),
          ]),
...
class _FaqsTabState extends State<_FaqsTab> {
  ...
  Future<void> _save() async {
    ...
    final order = int.tryParse(_orderCtrl.text.trim()) ?? 99;
    if (question.isEmpty || answer.isEmpty) {
      _snack('Question and answer are required.', isError: true);
      return;
    }
    String? err;
    if (_editing != null) {
      err = await FirebaseService.updateFaq(_editing!.docId!,
          FaqEntry(question: question, answer: answer, order: order));
      if (err == null) {
        setState(() => _editing = null);
        _snack('FAQ updated!');
      }
    } else {
      err = await FirebaseService.addFaq(
          FaqEntry(question: question, answer: answer, order: order));
      if (err == null) _snack('FAQ added!');
    }
    if (err != null) {
      _snack('Error: $err', isError: true);
      return;
  ...
  void _startEdit(FaqEntry f) {
    setState(() => _editing = f);
    _questionCtrl.text = f.question;
    _answerCtrl.text = f.answer;
    _orderCtrl.text = f.order.toString();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          _SectionTitle(
              title: _editing != null ? 'Edit FAQ' : 'Add FAQ',
              sub: 'Questions appear on the public FAQs page'),
          if (_editing != null)
            _EditingBanner(
                label: 'Editing: ${_editing!.question}',
                onCancel: () {
                  setState(() => _editing = null);
                  _questionCtrl.clear();
                  _answerCtrl.clear();
          ...
          _StyledTextField(
              controller: _questionCtrl, hint: 'How do I book a session?'),
          const SizedBox(height: 10),
          const _FieldLabel('Answer'),
          _StyledTextField(
              controller: _answerCtrl,
              hint: 'Short answer for visitors',
              maxLines: 3),
          const SizedBox(height: 10),
          const _FieldLabel('Order'),
          _StyledTextField(controller: _orderCtrl, hint: '99'),
          const SizedBox(height: 6),
          const Text(
            'Lower numbers appear first. Use this to control the display order on the public FAQ page.',
            style: TextStyle(color: AppTheme.grey, fontSize: 12, height: 1.4),
          ),
          const SizedBox(height: 20),
          _SaveButton(
              label: _editing != null ? 'Update FAQ' : 'Add FAQ', onTap: _save),
          const SizedBox(height: 40),
          const _SectionTitle(
              title: 'All FAQs', sub: 'Tap the edit icon to update or delete'),
          const SizedBox(height: 16),
          StreamBuilder<List<FaqEntry>>(
            stream: FirebaseService.faqsStream(),
            builder: (_, snap) {
              final items = snap.data ?? [];
              if (items.isEmpty) return const _EmptyBox(label: 'No FAQs yet');
              return Column(
                  children: items
                      .map((f) => _EditableRow(
                            label: f.question,
                            ...
                            onDelete: () async {
                              final ok = await showDialog<bool>(
                                      context: context,
                                      builder: (_) => const _ConfirmDialog(
                                          message: 'Remove this FAQ?')) ??
                                  false;
                              if (ok) await FirebaseService.removeFaq(f.docId!);
                            },
                          ))
                      .toList());
            },
...
class _ContactTabState extends State<_ContactTab> {
  ...
  Future<void> _load() async {
    try {
      final c = await FirebaseService.contactStream()
          .first
          .timeout(const Duration(seconds: 6), onTimeout: () => ContactData());
      if (!mounted) return;
  ...
  Future<void> _save() async {
    ...
    setState(() => _saving = false);
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(err != null ? 'Error: $err' : 'Contact info saved!',
        ...
        shape: RoundedRectangleBorder(
            ...
            side: BorderSide(
                color:
                    err != null ? const Color(0xFFFF5555) : AppTheme.border))));
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (!_loaded)
      return const Center(
          child:
              CircularProgressIndicator(color: AppTheme.white, strokeWidth: 1));
    return SingleChildScrollView(
        padding: EdgeInsets.all(isMobile ? 20 : 32),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const _SectionTitle(
          ...
          _StyledTextField(
              controller: _addressCtrl, hint: 'City, Province, Country'),
          const SizedBox(height: 12),
          const _FieldLabel('Instagram URL'),
          _StyledTextField(
              controller: _igCtrl, hint: 'https://instagram.com/...'),
          const SizedBox(height: 12),
          const _FieldLabel('Facebook URL'),
          _StyledTextField(
              controller: _fbCtrl, hint: 'https://facebook.com/...'),
          const SizedBox(height: 24),
          _SaveButton(
              label: 'Save Changes',
              loading: _saving,
...
class _SectionTitle extends StatelessWidget {
  ...
  Widget build(BuildContext context) =>
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Text(sub,
            style: const TextStyle(fontSize: 12, color: AppTheme.greyDark)),
      ]);
}
          ...
          child: Container(
              ...
              child: Text(label,
                  style: TextStyle(
                      ...
                      color: selected ? AppTheme.black : AppTheme.grey)))));
}
          ...
          child: Padding(
              ...
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                if (loading)
                  ...
                  const Icon(Icons.check, size: 15, color: AppTheme.black),
                const SizedBox(width: 7),
                Text(loading ? 'Saving…' : label,
                    style: TextStyle(
                        ...
                        fontSize: 13,
                        fontWeight: FontWeight.w700,
                        color: loading ? AppTheme.grey : AppTheme.black)),
              ]))));
}
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Row(children: [
          ...
          Expanded(
              child: Text(label,
                  style:
                      const TextStyle(fontSize: 12, color: Color(0xFFFBBF24)))),
          Material(
              color: Colors.transparent,
              child: InkWell(
                  onTap: onCancel,
        ...
        child: Row(children: [
          Expanded(
              child: Column(
                ...
                Text(sublabel,
                    style:
                        const TextStyle(fontSize: 11, color: AppTheme.greyDark),
                    maxLines: 1,
              ...
              child: InkWell(
                  ...
                  child: Container(
                      ...
                      child: const Icon(Icons.delete_outline,
                          size: 14, color: Color(0xFFFF5555))))),
        ]),
      );
}
          ...
          child: AnimatedContainer(
              ...
              child: Column(
                  ...
                  children: [
                    ...
                    const Text('Up to 10 photos',
                        style:
                            TextStyle(fontSize: 11, color: AppTheme.greyDark)),
                  ]))));
}
...
class _UploadTask {
  ...
  const _UploadTask(
      {required this.name,
      required this.bytes,
      this.status = _UStatus.pending,
      this.error});
  _UploadTask copyWith({_UStatus? status, String? error}) => _UploadTask(
      name: name,
      bytes: bytes,
          ...
          child: Row(children: [
            SizedBox(
                ...
                child: switch (task.status) {
                  _UStatus.pending => const Icon(Icons.schedule,
                      size: 15, color: AppTheme.greyDark),
                  _UStatus.uploading => const SizedBox(
                      width: 15,
                      height: 15,
                      child: CircularProgressIndicator(
                          strokeWidth: 1.5, color: AppTheme.white)),
                  _UStatus.done => const Icon(Icons.check_circle_outline,
                      size: 15, color: Color(0xFF4ADE80)),
                  _UStatus.error => const Icon(Icons.error_outline,
                      size: 15, color: Color(0xFFFF5555)),
                }),
            ...
            Expanded(
                child: Column(
                  ...
                  Text(task.name,
                      style:
                          const TextStyle(fontSize: 13, color: AppTheme.white),
                      overflow: TextOverflow.ellipsis),
                  if (task.error != null)
                    Text(task.error!,
                        style: const TextStyle(
                            fontSize: 11, color: Color(0xFFFF5555))),
                ])),
        ...
        return SingleChildScrollView(
          ...
          child:
            ...
            if (snap.connectionState == ConnectionState.waiting)
              ...
              ...messages
                  .map((m) => _MessageCard(message: m, context: context)),
            const SizedBox(height: 60),
          ]),
        );
      },
          ...
          if (!_expanded)
            Padding(
              ...
              child: Text(m.message,
                  ...
                  overflow: TextOverflow.ellipsis,
                  style:
                      const TextStyle(fontSize: 13, color: AppTheme.greyDark)),
            ),
                          ...
                          child: InkWell(
                            onTap: () async {
                              final uri = Uri(
                                  ...
                                  path: m.email,
                                  queryParameters: {
                                    'subject': 'Re: Your message via portfolio'
                                  });
                              await launchUrl(uri);
                            },
                            borderRadius: BorderRadius.circular(3),
