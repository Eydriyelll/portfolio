...
class FirebaseService {
  ...
  static Future<String?> updateSkill(String id, SkillEntry s) async {
    try {
      await _db
          .collection('skills')
          .doc(id)
  ...
  static Future<String?> updateCert(String id, CertEntry c) async {
    try {
      await _db
          .collection('certificates')
          .doc(id)
          .update({'name': c.name, 'issuer': c.issuer, 'imageUrl': c.imageUrl});
      return null;
    } catch (e) {
      return e.toString();
    }
  ...
  static Future<String?> updateProject(String id, ProjectEntry p) async {
    try {
  ...
  // HOBBIES
  static Stream<List<HobbyEntry>> hobbiesStream() => _db
      .collection('hobbies')
      .orderBy('order', descending: false)
      .snapshots()
      .map((s) => s.docs.map(HobbyEntry.fromDoc).toList());
  static Future<String?> addHobby(HobbyEntry h) async {
    try {
      await _db.collection('hobbies').add(h.toMap());
      return null;
    } catch (e) {
      return e.toString();
    }
  ...
  static Future<String?> updateHobby(String id, HobbyEntry h) async {
    try {
      await _db
          .collection('hobbies')
          .doc(id)
          .update({'name': h.name, 'desc': h.desc});
      return null;
    } catch (e) {
      return e.toString();
    }
  ...
  static Future<String?> removeHobby(String id) async {
    try {
  ...
  // REVIEWS
  static Stream<List<ReviewEntry>> reviewsStream() => _db
      .collection('reviews')
      .orderBy('createdAt', descending: true)
      .snapshots()
      .map((s) => s.docs.map(ReviewEntry.fromDoc).toList());
  static Future<String?> addReview(ReviewEntry r) async {
    try {
  ...
  static Future<String?> removeReview(String id) async {
    try {
  ...
  // FAQS
  static Stream<List<FaqEntry>> faqsStream() => _db
      .collection('faqs')
      .orderBy('order', descending: false)
      .snapshots()
      .map((s) => s.docs.map(FaqEntry.fromDoc).toList());
  static Future<String?> addFaq(FaqEntry f) async {
    try {
      await _db.collection('faqs').add(f.toMap());
      return null;
    } catch (e) {
      return e.toString();
    }
  ...
  static Future<String?> updateFaq(String id, FaqEntry f) async {
    try {
  ...
  static Future<String?> removeFaq(String id) async {
    try {
  ...
  // MESSAGES
  static Future<String?> sendMessage(MessageEntry m) async {
    try {
      await _db.collection('messages').add(m.toMap());
      return null;
    } catch (e) {
      return e.toString();
    }
  ...
  static Stream<List<MessageEntry>> messagesStream() => _db
      .collection('messages')
      .orderBy('sentAt', descending: true)
      .snapshots()
      .map((s) => s.docs.map(MessageEntry.fromDoc).toList());
  static Future<String?> deleteMessage(String id) async {
    try {
  ...
  static Future<String?> markMessageRead(String id) async {
    try {
      await _db.collection('messages').doc(id).update({'read': true});
      return null;
    } catch (e) {
      return e.toString();
    }
...
class SkillEntry {
  ...
  factory SkillEntry.fromDoc(DocumentSnapshot d) {
    ...
    return SkillEntry(
        docId: d.id,
        name: m['name'] ?? '',
        category: m['category'] ?? 'Other Skills');
  }
  Map<String, dynamic> toMap() => {
        'name': name,
        'category': category,
...
class CertEntry {
  ...
  CertEntry(
      {this.docId, required this.name, required this.issuer, this.imageUrl});
  factory CertEntry.fromDoc(DocumentSnapshot d) {
    final m = d.data() as Map<String, dynamic>;
    return CertEntry(
        docId: d.id,
        name: m['name'] ?? '',
        issuer: m['issuer'] ?? 'Other',
...
class HobbyEntry {
  ...
  final int order;
  HobbyEntry(
      {this.docId, required this.name, required this.desc, this.order = 99});
  factory HobbyEntry.fromDoc(DocumentSnapshot d) {
    final m = d.data() as Map<String, dynamic>;
    return HobbyEntry(
        docId: d.id,
        name: m['name'] ?? '',
...
class ReviewEntry {
  ...
  factory ReviewEntry.fromDoc(DocumentSnapshot d) {
    final m = d.data() as Map<String, dynamic>;
    return ReviewEntry(
      docId: d.id,
      name: m['name'] ?? 'Client',
      comment: m['comment'] ?? '',
      rating: (m['rating'] ?? 5).toInt(),
...
class FaqEntry {
  ...
  factory FaqEntry.fromDoc(DocumentSnapshot d) {
    final m = d.data() as Map<String, dynamic>;
    return FaqEntry(
      docId: d.id,
      question: m['question'] ?? '',
      answer: m['answer'] ?? '',
      order: (m['order'] ?? 99).toInt(),
    );
  }
